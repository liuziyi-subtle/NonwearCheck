...
void AutoRegressionPy(double *inputseries, int length, int degree,
  ...
  AutoRegression(inputseries, length, degree, coefficients, method);

  return;
}

int AutoRegression(double *inputseries, int length, int degree,
                   double *coefficients, int method) {
  double mean;
  int i, t;
  ...
  mean = 0.0;
  for (t = 0; t < length; t++) mean += inputseries[t];
  mean /= (double)length;
  for (t = 0; t < length; t++) w[t] = inputseries[t] - mean;
  ...
  if (method == MAXENTROPY) {
    ...
    for (i = 1; i <= degree; i++) coefficients[i - 1] = -ar[degree][i];
  ...
  if (h != NULL) free(h);
  if (g != NULL) free(g);
  if (per != NULL) free(per);
  if (pef != NULL) free(pef);
  if (ar != NULL) {
    for (i = 0; i < degree + 1; i++)
...
int ARMaxEntropy(double *inputseries, int length, int degree, double **ar,
  ...
  for (nn = 2; nn <= degree + 1; nn++) {
    ...
    t1 = g[nn];
    if (n != 0) {
      for (j = 2; j < nn; j++) h[j] = g[j] + (t1 * g[n - j + 3]);
      for (j = 2; j < nn; j++) g[j] = h[j];
      jj--;
    }
    for (j = 1; j <= jj; j++) {
    ...
    for (j = 2; j <= nn; j++) ar[nn - 1][j - 1] = g[j];
  }
...
*/
int ARLeastSquare(double *inputseries, int length, int degree,
                  double *coefficients) {
  int i, j, k, hj, hi;
  double **mat;
