...
def ar_feature(df_objects):
    ...
    for obj_id in df_objects['id'].unique():
        ...
        # print("=======")
        print(obj)
        inputseries = ar.double_array(64)
        for i in range(64):
            inputseries[i] = float(obj[i])
        coefficients = ar.double_array(10)
        coefficients_list = []
        ar.AutoRegressionPy(inputseries, 64, 10, coefficients, 1)
        for i in range(10):
...
if __name__ == '__main__':
    ...
    python3 ./extract_features.py --object_path ../../data/nonwear-check/R/results/objects_ppg-g_scale.csv \
                                  --feature_path ../../data/nonwear-check/R/results/features_ppg-g_scale-ar.csv
    '''
    import argparse
